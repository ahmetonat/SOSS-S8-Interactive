...
#define REF_VAL_2 1624   //Reference angle 2. (in encoder counts) HP motor.
//#define REF_VAL_2 900   //Reference angle 2. (in encoder counts) JP servo
...
//UART buffered read:
__IO uint8_t rcv_buf[RCVBUFLEN]; //UART receive buffer.
__IO uint8_t rcvbuf_wr;          //UART buf write pointer
__IO uint8_t rcvbuf_rd;          //UART buf read pointer
...
typedef struct {
  ...
  uint8_t ref_mode;  // Determines the source of the reference signal.
          //Ref Mode: 0: from command, 1: Step, periodic. 2: potentiometer.
  uint16_t ref_peri;
}ControlValues_t;
...
void main(){     
  ...
  ParserParams_t user;  //Command parser init.
  ...
  //12V, Ts=.5ms; Kp=100, Ki=25, Kd=20000, Ki>>5, Kd>>4 is good.
  //HP:12V, Ts=.5ms; Kp=50, Ki=25, Kd=10000, Ki>>5, Kd>>4 is good.
  ...
  ctrl.ref_mode=REFMODE_PER;  //Initially, periodic reference.
  ctrl.ref_peri=REF_PERIOD;  //Initially, periodic reference.
  ...
  init_parser (&user); // The command parser initialization.

  rcvbuf_wr=0; //UART buffer pointers initialized.
  rcvbuf_rd=0;
  UART1_Config(); 
...
//#ifdef ANALOG_REFERENCE
	...
	ADC1->CR1 |= ADC1_CR1_ADON;//Start ADC1 conversion for next ref sample.
      }else if (ctrl.ref_mode==REFMODE_PER){
	//#else
	ctrl.ref=references[ref_seq_no]; //Assign the current reference.
	//#endif
      }else{
	ctrl.ref=references[1]; //TODO: Fix to a proper variable.
	                             //This is DANGEROUS!
      }
    ...
    if(REF_count>=ctrl.ref_peri){ //If time to change reference val has come,
    ...
    // UART commands receive and parse:
  
    if (rcvbuf_rd!=rcvbuf_wr){ //If pointers mismatch, new chars have arrived.
      //Call the command parse state machine here.

      user.rcv_chr=rcv_buf[rcvbuf_rd]; //Get the next character.
      
      UART1->DR=user.rcv_chr;  //Echo the received character.
      while ((UART1->SR & (uint8_t)UART1_FLAG_TXE) == (uint8_t)0x00);
      
      
      parser(&user); //Parse the incoming stream.

      if (user.cmd_parsed==TRUE){ //If parsing was successful:

	//TODO: Assign the correct values based on the command.
	//ctrl.ref=(int16_t)(user.cmd_arg);
	if (user.command==CMD_REF){
	references[1]=(int16_t)(user.cmd_arg);
	}else if (user.command==CMD_TS){ //Set ref period command(in ms).
	  ctrl.ref_peri=(uint16_t)(user.cmd_arg);
	}else if (user.command==CMD_KP){ //Set Kp command.
	  ctrl.Kp=(int16_t)(user.cmd_arg);
	  //Ooops, no Kp_div defined...
	}else if (user.command==CMD_KD){ //Set Kd command.
	  ctrl.Kd=(int16_t)(user.cmd_arg);
	  ctrl.Kd_div=(int8_t)(user.exp);
	}else if (user.command==CMD_KI){ //Set Ki command.
	  ctrl.Ki=(int16_t)(user.cmd_arg);
	  ctrl.Ki_div=(int8_t)(user.exp);
	}else if (user.command==CMD_MOD){ //Set ref mode command.
	  if(user.cmd_arg==REFMODE_CMD){
	    ctrl.ref_mode=REFMODE_CMD;  //Initially, periodic reference.
	  }  else if(user.cmd_arg==REFMODE_PER){
	    ctrl.ref_mode=REFMODE_PER;  //Initially, periodic reference.
	  }  else if(user.cmd_arg==REFMODE_POT){
	    ctrl.ref_mode=REFMODE_POT;  //Initially, periodic reference.
	  }
	}
	//* TODO: Prevent echo in normal ops.
	UART1->DR=user.cmd_arg; //Echo back the received arg. //TODO:remove.
	while ((UART1->SR & (uint8_t)UART1_FLAG_TXE) == (uint8_t)0x00);
	UART1->DR='\r';//TODO: remove.
	while ((UART1->SR & (uint8_t)UART1_FLAG_TXE) == (uint8_t)0x00);
	UART1->DR=('\n');//TODO: remove.
	while ((UART1->SR & (uint8_t)UART1_FLAG_TXE) == (uint8_t)0x00);
	//*/
	user.cmd_parsed=FALSE;

	//TODO: Reset the parser here!

      }
      
      //chr_received=FALSE;
      ++rcvbuf_rd; //Point to next received char.
      if (rcvbuf_rd>=RCVBUFLEN){ //Wrap buffer.
	rcvbuf_rd=0;
      }
    }
    ...
    if(printing==TRUE){ //If a print operation has been initiated,
      ...
      //Not implemented as ISR to avoid interfering with control timing.
